	...
	function connect($db_host, $db_username, $db_password, $db_name, $use_names = "", $pconnect = true, $newlink = false)
  {
		...
		if ( $db_host != "" )
    {
      ...
      $connect_str .= "password=".$db_password;
		...
		if ( $pconnect )
      $this->link_id = @pg_pconnect($connect_str, ( ( $newlink ) ? PGSQL_CONNECT_FORCE_NEW : NULL ));
		else
      $this->link_id = @pg_connect($connect_str, ( ( $newlink ) ? PGSQL_CONNECT_FORCE_NEW : NULL ));

		if ( !$this->link_id )
      error($lang_global["err_sql_conn_db"]);
    else
    {
      if ( !empty($use_names) )
        pg_set_client_encoding($this->link_id, $use_names);
      return $this->link_id;
    }
	}
