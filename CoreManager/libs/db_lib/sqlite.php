...
class SQL //MySQL Lite
{
	...
	function connect($db_host, $db_username, $db_password, $db_name, $use_names = "", $pconnect = true, $newlink = false)
  {
		...
		if ( !file_exists($db_name) )
    {
		...
		if ( ( !is_readable($db_name) ) || ( !is_writable($db_name) ) )
      error($lang_global["err_sql_conn_db"]);

		if ( $pconnect )
      $this->link_id = @sqlite_popen($db_name, 0666, $this->sqlite_error);
		else
      $this->link_id = @sqlite_open($db_name, 0666, $this->sqlite_error);
