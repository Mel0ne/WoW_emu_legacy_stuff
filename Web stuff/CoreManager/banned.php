  ...
  switch ( $ban_type )
  {
    case "accounts":
    {
      if ( $core == 1 )
        ...
        $result = $sql["logon"]->query("SELECT id AS acct, unbandate AS banned, bannedby, banreason FROM account_banned WHERE active<>0 AND unbandate>UNIX_TIMESTAMP() ORDER BY ".$order_by." ".$order_dir." LIMIT ".$start.", ".$itemperpage);
      break;
    }
    case "characters":
    {
    ...
    case "ipbans":
    {
      if ( $core == 1 )
        ...
        $result = $sql["logon"]->query("SELECT ip, unbandate AS expire, bannedby, banreason FROM ip_banned ORDER BY ".$order_by." ".$order_dir." LIMIT ".$start.", ".$itemperpage);
      break;
    }
  }
...
{
  global $logon_db, $characters_db, $realm_id, $action_permission, $user_lvl, $sql, $core;
  ...
  switch ( $ban_type )
  {
    case "accounts":
    {
      if ( $core == 1 )
        ...
        $sql["logon"]->query("UPDATE account_banned SET active='0' WHERE id='".$entry."'");
    ...
    case "ipbans":
    {
      if ( $core == 1 )
        ...
        $sql["logon"]->query("DELETE FROM ip_banned WHERE ip='".$entry."'");
...
function add_entry()
{
  global $output, $action_permission, $user_lvl, $core;
